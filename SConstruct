...
def modify_compile(target, source, env):
    cmpBld = env.get_builder('AdaCompile')
    target.append(re.sub(cmpBld.get_suffix(env), '.o', str(target[0])))
    return target, source


# target for bind is the executable name
def modify_bind(target, source, env):
    # retrieve bind builder
    bndBld = env.get_builder('AdaBind')
    target = [re.sub(bndBld.get_src_suffix(env), '', str(source[0]))]
    return target, source

# target for link is the executable name
def modify_link(target, source, env):
    # retrieve link builder
    bndBld = env.get_builder('AdaBind')
    target = [re.sub(bndBld.get_prefix(env), '', str(source[0]))]
